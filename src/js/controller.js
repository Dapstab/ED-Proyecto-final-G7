...
window.addEventListener("load", () => {
  if (
    !JSON.parse(localStorage.getItem("user")).activo ||
    !JSON.parse(localStorage.getItem("user"))
  ) {
    window.setTimeout(() => {
      location.assign("http://localhost:1234/signup.html");
    }, 1500);
  }
});
...
const controlRecipes = async function () {
  try {
    ...
    recipeView._addHandlerOrder(controlOrder);
  } catch (err) {
...
const controlSearchResults = async function () {
  try {
    ...
    await model.loadSearchResults(query); // No retorna nada solo cambia un estado.
